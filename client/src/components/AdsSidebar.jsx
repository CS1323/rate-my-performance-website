...
export function AdsSidebar({ adIndex }) {
  ...
  if (adIndex !== undefined && adIndex !== null) {
    ...
    return (
      <aside className="right sidebar">
        <a href={ad.link || '#'} target="_blank" rel="noopener noreferrer" onClick={() => gaEvent({ category: 'ads', action: 'ads_clicked', label: ad.id?.toString() ?? ad.link })}>
          <img
            className="ad-image"
            src={ad.imageUrl}
  ...
  return (
    <aside className="right sidebar">
      {ads.map((ad) => (
        <a key={ad.id} href={ad.link || '#'} target="_blank" rel="noopener noreferrer" onClick={() => gaEvent({ category: 'ads', action: 'ads_clicked', label: ad.id?.toString() ?? ad.link })}>
          <img
            className="ad-image"
            src={ad.imageUrl}
            alt={ad.alt || t('ads.imageAlt')}
